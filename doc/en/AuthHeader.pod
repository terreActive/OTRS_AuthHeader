...
The following example is for Ubuntu's /etc/apache2/site-available 
mechanism:
    ...
    <VirtualHost *:443>
        ...
        <IfModule mod_perl.c>
            # parse headers only for selected scripts
            <Location /otrs>
                AuthType ...
                AuthName "...
                Require valid-user
            ...
            <Location /otrs/index.pl>
                SetEnvIf X_OTRS_AGENT "^(.*)$" X_OTRS_AGENT=$1
                PerlOptions +ParseHeaders
                PerlOptions +SetupEnv
            </Location>
            # Customer user
            <Location /otrs/customer.pl>
                SetEnvIf X_OTRS_AGENT "^(.*)$" X_OTRS_CUSTOMERUSER=$1
                PerlOptions +ParseHeaders
                PerlOptions +SetupEnv
            </Location>
        </IfModule>
...
Enable the apache configuration provided by OTRS:

    ln -s /opt/otrs/scripts/apache2-httpd.include.conf /etc/apache2/conf-available/
    a2enconf apache2-httpd.include
    apache2ctl restart

=head1 COPYRIGHT AND LICENCE

Copyright (c) 2022 by Othmar Wigger <othmar.wigger@terreactive.ch>
