# --
# Kernel/System/CustomerAuth/Header.pm - header based customer authentication
...
package Kernel::System::CustomerAuth::Header;
...
sub new {
    ...
    for (qw(Env Die)) {
        if ($ConfigObject->Get("Customer::AuthModule::Header::$_$Param{Count}")) {
            $Self->{$_} = $ConfigObject->Get("Customer::AuthModule::Header::$_$Param{Count}" );
        }
    }
    return $Self;
}
...
sub Auth {
    ...
    # query user is in DB
    my $UserID;
    my $SQL = "SELECT login FROM customer_user WHERE valid_id=1 AND login='$User'";
